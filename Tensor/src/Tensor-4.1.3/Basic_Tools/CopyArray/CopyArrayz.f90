	subroutine CopyArrayz(outA,inA,length)
		integer,intent(in)::length
		complex*16,intent(inout)::outA(:)
		class(*),intent(in)::inA(:)
		integer::i
		select type(inA)
			type is (integer)
				if(length.gt.LAPACK_LENGTH)then
					call zcopy(length, dcmplx(inA(1:length)), 1, outA, 1)
				else
			...
			type is (complex(kind=8))
				if(length.gt.LAPACK_LENGTH)then
					call zcopy(length, inA(1:length), 1, outA, 1)
				else
					outA(1:length)=inA(1:length)
				end if
			type is (logical)
