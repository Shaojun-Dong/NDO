...
contains
	...
	subroutine writeout1(p,w,uni)
		...
		select case(ptype)
			...
			case(7)
				do i=1,p%length
					write(uni,*)"    ",trim(p%name%ai(i)),"    ",trim("'"+trim(p%parameter%ai(i))+"'")
				end do
		end select
	end subroutine
	...
	subroutine printParameter3(p)
		...
		character(len=20)::F
		integer::i,ptype
		if(p%parameter%getType().eq.7)then
			lenA(2)=len_trim(p%parameter%ai(1))
			do i=2,p%parameter%getTotalData()
				if(len_trim(p%parameter%ai(i)).gt.lenA(2))lenA(2)=len_trim(p%parameter%ai(i))
			end do
		else
			lenA(2)=50
		end if
		lenA(1)=len_trim(p%name%ai(1))
		do i=2,p%parameter%getTotalData()
			if(len_trim(p%name%ai(i)).gt.lenA(1))lenA(1)=len_trim(p%name%ai(i))
		end do
		lenA=lenA+2
	...
	type(List) function AddParameter(List1,List2)
		...
		if(List1%length.eq.0)then
			AddParameter=List2
			return
		end if
		if(List2%length.eq.0)then
			AddParameter=List1
			return
		end if
	...
	function iList(Charname,indata)
		...
		character(len=characterlen)::words,tmpw
	...
	type(Tensor) function Arrayelement2(L,Cha)result(Res)
		...
		do k=1,LenData
			...
			call Res%setValue([i,j],temp(k),[1,temp(k)%getTotalData()])
		end do
		return
	end function 
		
		
	subroutine MPI_BCAST_parameter(P,ID,MPIcommon)
		type(List),intent(inout)::p
		integer,intent(in)::ID
		integer,optional,intent(in)::MPIcommon
		call MPI_BCAST_Tensor(p%parameter,ID,MPIcommon)
		call MPI_BCAST_Tensor(p%name,ID,MPIcommon)
		p%length=p%parameter%getTotalData()
		return
	end subroutine
	
	subroutine MPI_send_parameter(p1,p2,ID1,ID2,MPIcommon)
		type(List),intent(in)::p1
		type(List),intent(inout)::p2
		integer,intent(in)::ID1,ID2
		integer,optional,intent(in)::MPIcommon
		call MPI_send_Tensor(p1%parameter,p2%parameter,ID1,ID2,MPIcommon)
		call MPI_send_Tensor(p1%name,p2%name,ID1,ID2,MPIcommon)
